    ...
    def handle(self, *args, **options):
        ...
        for dividends_transaction in DividendsTransaction.objects.all():
            dividends_transaction.total_amount = (
                dividends_transaction.count
                * dividends_transaction.gross_price_per_share
            )
        ...
        for dividends_transaction in DividendsTransaction.objects.all():
            dividends_transaction.total_amount.currency = (
                dividends_transaction.company.dividends_currency
            )
            dividends_transaction.save()

        self.stdout.write(
            self.style.SUCCESS("Successfully set the dividends total amount")
        )
