...
import getRoute, { DIVIDENDS_EDIT_ROUTE } from "routes";
...
export default function DividendsListTable(): ReactElement {
  ...
  const columns: any = [
    ...
    {
      title: t("Gross dividend per share"),
    ...
    },
    {
      ...
      render: (text: string, record: any) => (
        <Space size="middle">
          <Link
            to={`${getRoute(DIVIDENDS_EDIT_ROUTE)
          ...
          <Popconfirm
            key={`transaction-delete-${record.key}`}
  ...
  const getData = () => {
    return company.dividendsTransactions.map(
